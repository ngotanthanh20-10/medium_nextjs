import Image from "next/image";

const Banner = () => {
  return (
    <div className="flex items-center justify-center bg-[#fcc017] border-y border-black p-2">
      <div className="max-w-7xl flex-1 flex items-center justify-between">
        <div className="space-y-5 px-10 flex-[3]">
          <h1 className="max-w-xl text-[3rem] xl:text-[6rem] lg:text-[5rem] sm:text-[4rem] font-mediumSerif">
            Stay Curious
          </h1>
          <h3 className="text-2xl">
            Discover stories, thinking, and expertise from writers on any topic
          </h3>
          <button className="bg-black text-white py-2 px-4 rounded-full">
            Start Reading
          </button>
        </div>
        <Image
          className="hidden h-32 md:inline-flex object-contain flex-1"
          src="/banner.png"
          alt=""
          height={400}
          width={500}
        />
      </div>
    </div>
  );
};

export default Banner;
